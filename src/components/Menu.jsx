...
export default function Menu({ items, categories, addToCart }) {
  const [activeCategory, setActiveCategory] = useState('all')
  const [search, setSearch] = useState('')
  const [lightboxItem, setLightboxItem] = useState(null)

  // Filter out "all" from filter buttons (it's a filter, not a real category)
  ...
  return (
    <section id="menu" className="py-20 bg-white relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Empty state */}
        {items.length === 0 && (
          <div className="text-center py-16 bg-spice-50 rounded-2xl">
            <div className="text-7xl mb-4">🍽️</div>
            <h3 className="font-display text-2xl font-bold text-spice-800 mb-2">No items yet</h3>
            <p className="text-gray-600 mb-4">The menu is being prepared. Please check back soon!</p>
            <a href="#admin" className="inline-block btn-primary text-sm">
              🔐 Admin: Add Items
            </a>
          </div>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="p-5">
                    ...
                    >
                      {item.available === false ? <>❌ Unavailable</> : <><span>➕</span> Add to Cart</>}
                    </button>
                  </div>
                </div>
              )
        ...
        {items.length > 0 && filtered.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            ...
            <p>No items match your search. Try a different category or search term.</p>
          </div>
        )}
      </div>
      ...
      {lightboxItem && (
        <Lightbox
          images={lightboxItem.images && lightboxItem.images.length > 0 ? lightboxItem.images : (lightboxItem.image ? [lightboxItem.image] : [])}
